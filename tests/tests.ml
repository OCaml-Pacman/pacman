...
let test_update_player _ =
  assert_equal true @@ (Game_map.load "map.csv" && Render.load_sprite "sprite.png");
  let player = Player.create (0.0, 0.0) in
  let updated_player = Player.update player (Key 'c') in
  assert_equal updated_player.move_counter @@ (player.move_counter + 1)
