...
let get_number_sprite (num : int) : int * int = (num, 8)
...
let convert_imagelib_to_graphics (img : Image.image) =
  ...
  for y = 0 to h - 1 do
    for x = 0 to w - 1 do
      g_img.(y).(x) <-
        Graphics.rgb
          (Image.Pixmap.get chan_r x y)
          (Image.Pixmap.get chan_g x y)
          (Image.Pixmap.get chan_b x y)
    done
  done;
...
let sub_image sheet x_pos y_pos width height transp =
  let x_pos, y_pos = (x_pos * resize_ratio, y_pos * resize_ratio) in
  let width, height = (width * resize_ratio, height * resize_ratio) in
  let sheet_matrix = Graphics.dump_image sheet in
  let sub_matrix = Array.make_matrix height width Graphics.transp in
  for y = 0 to height - 1 do
    for x = 0 to width - 1 do
      let color =
        sheet_matrix.((y_pos + y) / resize_ratio).((x_pos + x) / resize_ratio)
      in
      sub_matrix.(y).(x) <-
        (if color = Graphics.black && transp then Graphics.transp else color)
    done
  done;
...
let draw_sprite ?(transp = true) (sx, sy) (x, y) : unit =
  Graphics.draw_image (sprite_from_sheet !sprite sx sy transp) x y

let set_res _ =
  let fx, fy = get_size () in
  let x = Float.to_int fx * 16 * resize_ratio in
  let y = Float.to_int (fy +. 1.) * 16 * resize_ratio in
...
let coord_map2screen (x, y) =
  ...
  (x * resize_ratio, y * resize_ratio)

let draw_map _ =
  let fx, fy = get_size () in
  let lim_x, lim_y =
    (Float.round fx |> Float.to_int, Float.round fy |> Float.to_int)
  in
  for x = 0 to lim_x - 1 do
    for y = 0 to lim_y - 1 do
      ...
      | Enemy enemy ->
          draw_sprite ~transp:false (Enemy.get_enemy_sprite_by_type enemy) loc
      ...
      | Fruit fruit ->
          draw_sprite ~transp:false (Fruit.get_sprite_from_type fruit) loc
    done;
    let map_loc = (Float.of_int x, -1.0) in
    let loc = coord_map2screen map_loc in
    draw_sprite ~transp:false balck_sprite loc
  done

let int_to_digit_list (n : int) : int list =
  let rec aux n acc =
    if n < 10 then n :: acc else aux (n / 10) ((n mod 10) :: acc)
  in
  if n = 0 then [ 0 ]
  else if n > 0 then aux n []
  else failwith "Negative numbers are not supported"

let draw_score (score : int) : unit =
  let digit_list = int_to_digit_list score in
  for x = 0 to (List.length digit_list) - 1 do
      let digit = List.nth digit_list x in
      let map_loc = (Float.of_int x, -1.0) in
      let loc = coord_map2screen map_loc in
      (draw_sprite (get_number_sprite digit) loc;)
  done
