...
let enemy_resurrection_time = 400
...
module MakeEnemy (M : SetEnemyType) : Enemy = struct
  ...
  let update (cur_e : t) (player_pos : float * float) : t =
    ...
    | Active ->
        ...
        moved_e.sprite <-
          get_sprite_by_dir moved_e.move_direction animated_ind M.get_sprite;
        moved_e
    | Scared ->
...
module Set_red_enemy : SetEnemyType = struct
  ...
  let move (cur_e : t) (player_pos : float * float) : t =
    let rec aux () =
      ...
      let next_x =
        fst cur_e.position +. (dx *. cur_e.speed) +. (0.0 *. tempx)
      in
      let next_y =
        snd cur_e.position +. (dy *. cur_e.speed) +. (0.0 *. tempy)
      in
    ...
    let dx, dy = match_dir_to_ind d in
    let next_x = fst cur_e.position +. (dx *. cur_e.speed) in
    let next_y = snd cur_e.position +. (dy *. cur_e.speed) in
...
module Set_pink_enemy : SetEnemyType = struct
  ...
  let get_distance (player_pos : float * float) (enemy_pos : float * float) :
      float =
  ...
  let change_direction (enemy_pos : float * float) (dir : direction) :
      float * float =
  ...
  let get_best_dir (player_pos : float * float) (enemy_pos : float * float) :
      direction =
    let dir_list = [ Up; Down; Right; Left ] in
    let all_dist =
      List.map dir_list ~f:(fun d ->
          (d, get_distance player_pos (change_direction enemy_pos d)))
    in
    let best_dir =
      List.fold all_dist ~init:(List.nth_exn all_dist 0) ~f:(fun acc x ->
          if Float.( <= ) (snd acc) (snd x) then acc else x)
    in
    fst best_dir
...
module BFS = struct
  ...
  let create_node position direction_from_parent parent =
    { position; direction_from_parent; parent }
  ...
  let get_neighbors node =
    ...
    |> List.filter ~f:(fun pos ->
           ...
           if
             Float.between (fst (fst pos)) ~low:0.0 ~high:(x_max -. 1.0)
  ...
  let bfs start_pos end_pos =
    let rec loop queue visited =
      if Deque.is_empty queue then None
        ...
        if check_position_equal pos end_pos then Some head (* Path found *)
        else if Set.mem visited pos then loop queue visited
        else
  ...
  let get_best_dir (player_pos : float * float) (enemy_pos : float * float) :
    ...
    let find_end =
      bfs
        ( Float.round_nearest (fst enemy_pos),
          Float.round_nearest (snd enemy_pos) )
        ( Float.round_nearest (fst player_pos),
          Float.round_nearest (snd player_pos) )
    in
    match find_end with
    | Some e -> trace_back_for_direction e
    | None ->
        helper_dir
          (Random.self_init ();
           Random.int 4)
...
module Set_blue_enemy : SetEnemyType = struct
  ...
  let move (cur_e : t) (player_pos : float * float) : t =
    let selected_dir =
      match_dir_to_int (get_best_dir player_pos cur_e.position)
    in
    let dx, dy = match_dir_to_ind selected_dir in
    let next_x = fst cur_e.position +. (dx *. cur_e.speed) in
    let next_y = snd cur_e.position +. (dy *. cur_e.speed) in
...
module Set_orange_enemy : SetEnemyType = struct
  ...
  let speed_limit = 0.085

  let move (cur_e : t) (player_pos : float * float) : t =
    if !speed_timer mod 50 = 0 then (
      speed_timer := 1;
      if Float.( <= ) cur_e.speed speed_limit then
        cur_e.speed <- cur_e.speed +. 0.002)
